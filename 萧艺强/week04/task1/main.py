...
from model.bert_product import model_for_bert_product
...
@app.post("/v1/text-cls/bert-product")
def bert_product_classify(req: TextClassifyRequest) -> TextClassifyResponse:
    """
    利用TFIDF进行文本分类
    ...
    # info 日志
    try:
        response.classify_result = model_for_bert_product(req.request_text)
        response.error_msg = "ok"
    except Exception as err:
        # error 日志
        response.classify_result = ""
