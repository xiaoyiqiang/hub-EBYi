...
from config import BERT_MODEL_PERTRAINED_PATH, BERT_MODEL_PKL_PATH_PRODUCT, CATEGORY_NAME
...
model.load_state_dict(torch.load(BERT_MODEL_PKL_PATH_PRODUCT))
model.to(device)
...
def model_for_bert_product(request_text: Union[str, List[str]]) -> Union[str, List[str]]:
    ...
    print(pred)
    classify_result = [loaded_categories[x] for x in pred]
